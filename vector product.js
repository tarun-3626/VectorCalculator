...
let icap = (k2*j1 - j2*k1) 
    let jcap =  ((k2*I1 - I2*k1)>0)? "+"+(k2*I1 - I2*k1) :(k2*I1 - I2*k1)
    let kcap = ((j2*I1 -j1*I2)>0)? "+"+(j2*I1 -j1*I2) : (j2*I1 -j1*I2)
    
    
    aXb = (icap)+"i^" + (jcap)+"j^" + (kcap) +"k^"
...
let instraction = ()=>{
    ...
    try1.innerHTML = `<h2 style= "margin:1rem : ">Instruction</h2>
        ...
        <p>formula = a₁Xb₂ = i[j₁xk₂ - k₁xj₂] - j[i₁xk₂ - k₁xi₂] + k[i₁xj₂ - j₁xi₂]</p>
        <p> i = icap (î) </p>
        <p> j = jcap (ĵ) </p>
        <p> k = kcap (k^) </p>
